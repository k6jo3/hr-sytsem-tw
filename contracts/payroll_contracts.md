...
> **軟刪除策略說明：**
> - **薪資結構：** 使用 `active` 欄位（PO 欄位名，對應 DTO 的 `isActive` 屬性）
...
| :--- | :--- | :--- | :--- | :--- |
| PAY_QRY_S001 | 查詢員工薪資結構 | HR | `{"employeeId":"E001"}` | `employeeId = 'E001'`, `active = 'true'` |
| PAY_QRY_S002 | 查詢有效薪資結構 | HR | `{"isActive":true}` | `active = 'true'` |
| PAY_QRY_S003 | 查詢月薪制結構 | HR | `{"payrollSystem":"MONTHLY"}` | `payrollSystem = 'MONTHLY'`, `active = 'true'` |
| PAY_QRY_S004 | 查詢時薪制結構 | HR | `{"payrollSystem":"HOURLY"}` | `payrollSystem = 'HOURLY'`, `active = 'true'` |
| PAY_QRY_S005 | 組合條件查詢 | HR | `{"employeeId":"E001","isActive":true}` | `employeeId = 'E001'`, `active = 'true'` |
| PAY_QRY_S006 | 查詢停用的薪資結構 | HR | `{"employeeId":"E001","isActive":false}` | `employeeId = 'E001'`, `active = 'false'` |
...
**API 端點：** `POST /api/v1/payroll/legal-deductions`

**業務場景描述：**

HR 根據法院/行政機關扣押令，為員工建立法扣款記錄。建立後狀態為 ACTIVE，已扣金額為 0，剩餘金額等於扣押總額。

**測試合約：**

```json
{
  "scenarioId": "PAY_CMD_LD001",
  "apiEndpoint": "POST /api/v1/payroll/legal-deductions",
  "controller": "HR04LegalDeductionCmdController",
  "service": "CreateLegalDeductionServiceImpl",
  "permission": "payroll:legal-deduction:create",
  "request": {
    "employeeId": "emp-uuid-001",
    "courtOrderNumber": "112-司執-12345",
    "garnishmentType": "COURT_ORDER",
    "totalAmount": 500000,
    "priority": 1,
    "effectiveDate": "2026-01-15",
    "issuingAuthority": "台北地方法院",
    "caseNumber": "112年度司執字第12345號",
    "note": "民事債務強制執行"
  },
  "businessRules": [
    {"rule": "扣押總額必須大於 0"},
    ...
    {"rule": "設定 deductedAmount = 0"},
    {"rule": "設定 remainingAmount = totalAmount"}
  ],
  "expectedDataChanges": [
    {
      "action": "INSERT",
      "table": "legal_deductions",
      "count": 1,
      "assertions": [
        {"field": "deduction_id", "operator": "notNull"},
        {"field": "employee_id", "operator": "equals", "value": "emp-uuid-001"},
        {"field": "court_order_number", "operator": "equals", "value": "112-司執-12345"},
        {"field": "garnishment_type", "operator": "equals", "value": "COURT_ORDER"},
        {"field": "total_amount", "operator": "equals", "value": 500000},
        {"field": "deducted_amount", "operator": "equals", "value": 0},
        {"field": "remaining_amount", "operator": "equals", "value": 500000},
  ...
  "expectedEvents": [
    {
      "eventType": "LegalDeductionCreatedEvent",
      "payload": [
        {"field": "deductionId", "operator": "notNull"},
        {"field": "employeeId", "operator": "equals", "value": "emp-uuid-001"},
        {"field": "garnishmentType", "operator": "equals", "value": "COURT_ORDER"}
      ]
    }
  ],
  "expectedResponse": {
    "statusCode": 200,
    "requiredFields": [
      {"name": "deductionId", "type": "string", "notNull": true},
      {"name": "employeeId", "type": "string", "notNull": true},
      {"name": "courtOrderNumber", "type": "string", "notNull": true},
      {"name": "garnishmentType", "type": "string", "notNull": true},
      {"name": "totalAmount", "type": "number", "notNull": true},
      {"name": "deductedAmount", "type": "number", "notNull": true},
      {"name": "remainingAmount", "type": "number", "notNull": true},
      {"name": "priority", "type": "integer", "notNull": true},
      {"name": "effectiveDate", "type": "string", "notNull": true},
...
**API 端點：** `PUT /api/v1/payroll/legal-deductions/{id}/suspend`

**業務場景描述：**

法院暫緩執行時，HR 暫停該筆法扣款。僅 ACTIVE 狀態的法扣款可暫停。

**測試合約：**

```json
{
  "scenarioId": "PAY_CMD_LD002",
  "apiEndpoint": "PUT /api/v1/payroll/legal-deductions/{id}/suspend",
  "controller": "HR04LegalDeductionCmdController",
  "service": "SuspendLegalDeductionServiceImpl",
  "permission": "payroll:legal-deduction:write",
  "request": {
    "deductionId": "deduction-uuid-001"
  },
  "businessRules": [
    {"rule": "僅 ACTIVE 狀態的法扣款可暫停"},
    {"rule": "更新狀態為 SUSPENDED"}
  ],
  "expectedDataChanges": [
    {
      "action": "UPDATE",
...
**API 端點：** `PUT /api/v1/payroll/legal-deductions/{id}/resume`

**業務場景描述：**

暫停後重新執行扣款。僅 SUSPENDED 狀態的法扣款可恢復。

**測試合約：**

```json
{
  "scenarioId": "PAY_CMD_LD003",
  "apiEndpoint": "PUT /api/v1/payroll/legal-deductions/{id}/resume",
  "controller": "HR04LegalDeductionCmdController",
...
**API 端點：** `PUT /api/v1/payroll/legal-deductions/{id}/terminate`

**業務場景描述：**

法院撤銷扣押令，HR 終止該筆法扣款。任何狀態（ACTIVE、SUSPENDED）都可終止。

**測試合約：**

```json
{
  "scenarioId": "PAY_CMD_LD004",
  "apiEndpoint": "PUT /api/v1/payroll/legal-deductions/{id}/terminate",
  "controller": "HR04LegalDeductionCmdController",
  ...
  "businessRules": [
    {"rule": "任何非 TERMINATED 狀態的法扣款都可終止"},
    {"rule": "更新狀態為 TERMINATED"}
  ],
  "expectedDataChanges": [
    {
      "action": "UPDATE",
...
#### PAY_QRY_LD001: 查詢法扣款列表

**API 端點：** `GET /api/v1/payroll/legal-deductions`

**業務場景描述：**

HR 查詢法扣款列表，支援依員工 ID、狀態、扣款類型篩選。

**測試合約：**

```json
{
  "scenarioId": "PAY_QRY_LD001",
  "apiEndpoint": "GET /api/v1/payroll/legal-deductions",
  "controller": "HR04LegalDeductionQryController",
  "service": "GetLegalDeductionListServiceImpl",
  "permission": "payroll:legal-deduction:read",
  "request": {
    "employeeId": "emp-uuid-001",
    "status": "ACTIVE"
  },
  "expectedQueryFilters": [
    {"field": "employeeId", "operator": "=", "value": "emp-uuid-001"},
...
| 場景 ID | 測試描述 | 模擬角色 | 輸入 (Request) | 必須包含的過濾條件 |
| :--- | :--- | :--- | :--- | :--- |
| PAY_QRY_LD001 | 查詢員工執行中法扣款 | HR | `{"employeeId":"E001","status":"ACTIVE"}` | `employeeId = 'E001'`, `status = 'ACTIVE'` |

---

#### PAY_QRY_LD002: 查詢單筆法扣款

**API 端點：** `GET /api/v1/payroll/legal-deductions/{id}`

**業務場景描述：**

HR 查詢單筆法扣款的完整資訊，包含扣款歷史與法律依據。

**測試合約：**

```json
{
  "scenarioId": "PAY_QRY_LD002",
  "apiEndpoint": "GET /api/v1/payroll/legal-deductions/{id}",
  "controller": "HR04LegalDeductionQryController",
  ...
  "expectedResponse": {
    ...
    "requiredFields": [
      ...
      {"name": "effectiveDate", "type": "string", "notNull": true},
      {"name": "status", "type": "string", "notNull": true},
      {"name": "issuingAuthority", "type": "string", "notNull": false},
      {"name": "caseNumber", "type": "string", "notNull": false},
      {"name": "note", "type": "string", "notNull": false}
    ]
  }
}
```
...
└───────────┘                   │
                                │
    任何狀態 ──terminate()──▶ TERMINATED
```

| 狀態 | 說明 |
|:---|:---|
| ACTIVE | 執行中（每月薪資計算時扣款） |
| SUSPENDED | 暫停（法院暫緩執行） |
| COMPLETED | 已全額扣完 |
| TERMINATED | 終止（法院撤銷扣押令） |
...
| # | 端點 | 方法 | 場景 ID | 說明 |
|:---:|:---|:---:|:---|:---|
| 1 | `POST /api/v1/payroll/salary-advances` | POST | PAY_CMD_SA001 | 申請預借薪資 |
| 2 | `PUT /api/v1/payroll/salary-advances/{id}/approve` | PUT | PAY_CMD_SA002 | 核准預借薪資 |
| 3 | `PUT /api/v1/payroll/salary-advances/{id}/reject` | PUT | PAY_CMD_SA003 | 駁回預借薪資 |
| 4 | `PUT /api/v1/payroll/salary-advances/{id}/disburse` | PUT | PAY_CMD_SA004 | 撥款預借薪資 |
| 5 | `PUT /api/v1/payroll/salary-advances/{id}/cancel` | PUT | PAY_CMD_SA005 | 取消預借薪資 |
| 6 | `GET /api/v1/payroll/salary-advances` | GET | PAY_QRY_SA001 | 查詢預借薪資列表 |
| 7 | `GET /api/v1/payroll/salary-advances/{id}` | GET | PAY_QRY_SA002 | 查詢單筆預借薪資 |
...
```json
{
  ...
  "request": {},
  "businessRules": [
    {"rule": "PENDING/APPROVED/REJECTED 可取消"},
    {"rule": "DISBURSED/REPAYING/FULLY_REPAID 不可取消"}
  ],
  "expectedDataChanges": [
    {
      "action": "UPDATE",
...
```json
{
  ...
  "request": {"employeeId": "emp-uuid-001", "status": "PENDING"},
  "expectedQueryFilters": [
...
**文件建立日期:** 2026-02-23
**版本:** 1.3（2026-03-16 新增法扣款 + 預借薪資合約）
