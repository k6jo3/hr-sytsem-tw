...
> **📄 詳細內容請參閱:** [系統架構設計文件_命名規範.md](./系統架構設計文件_命名規範.md)

本節詳細說明本專案的程式碼命名規範，確保團隊開發時保持一致性與可維護性。

**核心規範摘要:**

#### 5.5.1 Controller命名格式
```
HR{Domain代號}{畫面名稱}{Controller類型}Controller
```

**範例:**
| Domain | Controller名稱 | 說明 |
|:---|:---|:---|
...
#### 5.5.2 Query與Command Controller建議拆分
...
#### 5.5.4 Domain Service以領域事件命名
```
{業務動詞/事件名稱}DomainService
```
...
| `SalaryCalculationDomainService` | 薪資計算邏輯 |

---

### 5.6 Business Pipeline 模式

> **詳細架構:** 請參閱 [framework/architecture/03_Business_Pipeline.md](../framework/architecture/03_Business_Pipeline.md)

當 Service 包含複雜業務邏輯（3步驟以上、多資料來源）時，使用 Pipeline 模式將邏輯拆解為可測試、可觀測的小零件。
...
| 組件 | 職責 | 命名格式 |
|:---|:---|:---|
| **Context** | 傳遞輸入、中間結果、輸出 | `{UseCase}Context` |
| **Task** | 單一職責的執行步驟 | `Load{Entity}Task` / `{動詞}{業務}Task` |
...
public class CalculateMonthlySalaryServiceImpl
    ...
    protected SalaryResponse doExecute(CalculateSalaryRequest req, JWTModel user) {
        ...
        BusinessPipeline.start(ctx)
            ...
            .nextIf(c -> c.hasOvertime(), loadOvertimeTask)  // 條件分支
...
| 適用場景 | 不適用場景 |
|:---|:---|
| ✅ 複雜業務流程（薪資計算、訂單處理） | ❌ 簡單 CRUD 操作 |
| ✅ 多步驟資料載入與計算 | ❌ 只有 1-2 個步驟的邏輯 |
| ✅ 需要高可測試性的核心邏輯 | ❌ 單純的資料查詢 |
