...
**版本:** 1.1
**日期:** 2025-11-24（[2026-03-17 更新] 新增資遣費計算與離職結算需求）
...
- [2026-03-17 更新] 離職最後薪資結算（當月薪資 + 特休折算 + 資遣費 + 預告期工資）
...
| SeverancePay | 資遣費（[2026-03-17 更新]） | 依新制/舊制計算的資遣費用 |
| FinalSettlement | 離職結算（[2026-03-17 更新]） | 最後薪資 + 特休折算 + 資遣費 |
| AverageSalary | 平均工資（[2026-03-17 更新]） | 離職前6個月工資總額 / 6 |
| NoticePeriodPay | 預告期間工資（[2026-03-17 更新]） | 未足額預告時應補發的工資 |
...
| `FinalSettlementCreated`（[2026-03-17 更新]） | 離職結算單建立 | settlementId, employeeId, terminationType, netAmount | Notification |
| `SeverancePayCalculated`（[2026-03-17 更新]） | 資遣費計算完成 | employeeId, severanceAmount, calculationMethod | Notification, Report |
...
到職日：2022-06-01
離職日：2025-12-31
年資：3 年 7 個月 = 3.5833 年
平均工資：55,000 元

資遣費 = 55,000 x 0.5 x 3.5833 = 98,541 元
（未超過上限 55,000 x 6 = 330,000 元）
```
...
**觸發方式：** 接收 `EmployeeTerminatedEvent` 後自動執行

**結算項目（依 terminationType 決定）：**

| 結算項目 | 自願離職 | 資遣 | 懲戒解雇 | 退休 |
|:---|:---:|:---:|:---:|:---:|
| 當月薪資（按比例） | 是 | 是 | 是 | 是 |
| 特休折算工資 | 是 | 是 | 是 | 是 |
| 補休折算工資 | 是 | 是 | 是 | 是 |
...
```
最後實發 = 當月按比例薪資
         + 特休折算金額（由 Attendance 提供）
         + 補休折算金額（由 Attendance 提供）
         + 資遣費（僅 LAYOFF）
         + 預告期間工資（僅 LAYOFF 且未足額預告時）
         - 勞健保費用
         - 所得稅
```
...
```
POST /api/v1/payroll/final-settlement
Authorization: Bearer {token}
Required Permission: payroll:settlement:create
...
Request:
{
  ...
  "hireDate": "2022-06-01",
  "unusedAnnualLeaveDays": 5.5,
  "unusedCompensatoryLeaveDays": 2,
  "noticePeriodDays": 30,
  "actualNoticeDays": 20
}
...
Response 201:
{
  ...
  "employeeId": "uuid",
  "items": [
    {"itemName": "當月薪資（按比例）", "amount": 55000},
    {"itemName": "特休折算工資", "amount": 10083},
    {"itemName": "補休折算工資", "amount": 3667},
    {"itemName": "資遣費（新制）", "amount": 98541},
    {"itemName": "預告期間工資（10天差額）", "amount": 18333}
  ],
  "grossAmount": 185624,
  "deductions": [
    {"itemName": "勞保費", "amount": 1200},
    {"itemName": "健保費", "amount": 800},
    {"itemName": "所得稅", "amount": 5000}
  ],
...
| 規則代碼 | 規則描述 | 影響範圍 |
|:---|:---|:---|
| BR-04-101 | 資遣費新制：每年 0.5 個月平均工資，上限 6 個月 | 資遣費計算 |
| BR-04-102 | 資遣費舊制：每年 1 個月平均工資，無上限 | 資遣費計算 |
| BR-04-103 | 平均工資 = 離職前 6 個月工資總額 / 6 | 資遣費、預告期工資 |
| BR-04-104 | 預告期工資 = 平均工資 / 30 x 不足天數 | 預告期工資 |
| BR-04-105 | 特休折算 = 未休天數 x 日薪（平均工資 / 30） | 離職結算 |
| BR-04-106 | 離職當月薪資按日曆天比例計算 | 離職結算 |
...
**[2026-03-17 更新] 離職結算事件流程:**
```
1. Organization Service 發布 EmployeeTerminatedEvent
2. Payroll Service 訂閱事件
3. 依 terminationType 判斷結算項目：
   a. 查詢離職前 6 個月薪資歷史 → 計算平均工資
   b. 計算當月按比例薪資
   c. 接收 Attendance Service 的特休/補休折算金額（UnusedLeaveCalculated 事件）
   d. 若 terminationType == LAYOFF：
      - 計算資遣費（新制/舊制/混合制）
      - 計算預告期間工資差額（若未足額預告）
   e. 產生離職結算薪資單（FinalSettlement）
4. 發布 FinalSettlementCreated 事件
```
...
- **加班費計算符合勞基法:** 100%
- **所得稅扣繳符合財政部規定:** 100%
- [2026-03-17 更新] **資遣費計算符合勞退條例/勞基法:** 100%
- [2026-03-17 更新] **離職結算準確率:** 100%
