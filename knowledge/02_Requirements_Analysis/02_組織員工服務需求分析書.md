...
**版本:** 1.1
**日期:** 2025-11-24（[2026-03-17 更新] 新增離職類型區分與法規要件）
...
| TerminationType | 離職類型（[2026-03-17 更新]） | 自願離職、資遣、懲戒解雇、合意終止、契約到期、退休 |
| NoticePeriod | 預告期間（[2026-03-17 更新]） | 依年資計算的法定預告天數 |
| SeverancePay | 資遣費（[2026-03-17 更新]） | 依新制/舊制/混合制計算的資遣費 |
| InvoluntarySeparationCertificate | 非自願離職證明（[2026-03-17 更新]） | 資遣時依法開立 |
| Contract | 員工合約 | 不定期契約、定期契約 |
| Onboarding | 到職流程 | 新人報到、設備申請、教育訓練 |
| Termination | 離職流程 | 離職申請、交接、結算 |
| LayoffNotice | 資遣預告（[2026-03-17 更新]） | 雇主依法需提前通知的期間 |
| LayoffReport | 資遣通報（[2026-03-17 更新]） | 依就業服務法向勞動局通報 |
...
// [2026-03-17 更新] 新增離職類型枚舉
// 法規依據：勞動基準法第 11、12、14、15、53、54 條
enum TerminationType {
  VOLUNTARY_RESIGNATION   // 自願離職（勞基法第 15 條）
  LAYOFF                  // 資遣/經濟性解雇（勞基法第 11 條）
  DISMISSAL               // 懲戒性解雇（勞基法第 12 條）
  MUTUAL_AGREEMENT        // 合意終止
  CONTRACT_EXPIRY         // 定期契約到期
  RETIREMENT              // 退休（勞基法第 53、54 條）
}
```

**[2026-03-17 更新] 各離職類型的法律效果差異:**

| 離職類型 | 預告期 | 資遣費 | 非自願離職證明 | 謀職假 | 資遣通報 |
|:---|:---:|:---:|:---:|:---:|:---:|
| 自願離職 (VOLUNTARY_RESIGNATION) | 員工需預告 | 否 | 否 | 否 | 否 |
| 資遣 (LAYOFF) | 雇主需預告 | 是 | 是 | 是 | 是 |
| 懲戒解雇 (DISMISSAL) | 不需預告 | 否 | 視情況 | 否 | 否 |
| 合意終止 (MUTUAL_AGREEMENT) | 協議 | 協議 | 視協議 | 否 | 否 |
| 契約到期 (CONTRACT_EXPIRY) | 不需 | 否 | 否 | 否 | 否 |
| 退休 (RETIREMENT) | 依規定 | 否（有退休金） | 否 | 否 | 否 |
...
- 離職日期不可早於到職日期
- 試用期結束日必須晚於到職日
- [2026-03-17 更新] 資遣（LAYOFF）時必須填寫 terminationType
- [2026-03-17 更新] 資遣時系統須自動計算預告期與資遣費
- [2026-03-17 更新] 資遣時須產生非自願離職證明

**領域行為:**
- `onboard(hireDate, probationMonths)`: 到職
- `completeProbation()`: 通過試用期轉正
- `terminate(terminationDate, reason)`: 離職（舊介面，保留向下相容）
- [2026-03-17 更新] `terminate(terminationDate, terminationType, reason)`: 離職（含離職類型）
- [2026-03-17 更新] `calculateNoticePeriod()`: 計算資遣預告期天數
- [2026-03-17 更新] `calculateSeverancePay(averageSalary)`: 計算資遣費（委派 Payroll 模組）
- [2026-03-17 更新] `getServiceYears()`: 計算年資（到職日至離職日）
...
| `EmployeeTerminated` | 員工離職 | employeeId, terminationDate, terminationType, reason, hireDate, averageSalary | IAM, Attendance, Insurance, Payroll, Project, Timesheet |
...
**[2026-03-17 更新] Request 欄位說明:**

| 欄位 | 型別 | 必填 | 說明 |
|:---|:---|:---:|:---|
| terminationDate | Date | 是 | 離職生效日 |
| terminationType | TerminationType | 是 | 離職類型（見枚舉定義） |
| reason | String | 是 | 離職原因 |

**[2026-03-17 更新] 依離職類型觸發不同後續動作:**

**通用後續動作（所有類型）:**
- 發布 `EmployeeTerminated` 事件（**關鍵事件**）
- IAM Service 停用使用者帳號
- Insurance Service 辦理退保（勞保、健保、勞退）
- Attendance Service 結算特休（折算工資金額）、結算補休
- Payroll Service 執行最後薪資結算（當月薪資 + 特休折算）
- Project Service 移除專案成員或標記
- Timesheet Service 工時結算
- 記錄至 EmployeeHistory

**資遣 (LAYOFF) 專屬後續動作:**
- 系統自動計算預告期天數（BR-02-101）
- Payroll Service 計算資遣費（BR-02-102）
- 最後薪資包含：當月薪資 + 特休折算 + 資遣費 + 預告期工資（若未足額預告）
- Document Service 產生非自願離職證明書（BR-02-104）
- 系統提醒 HR 進行資遣通報（離職日 10 天前，BR-02-105）
- 員工享有謀職假權利（每週 2 日，BR-02-106）

**懲戒解雇 (DISMISSAL) 專屬後續動作:**
- 不需預告期、不需資遣費
- 視情況決定是否開立非自願離職證明

**退休 (RETIREMENT) 專屬後續動作:**
- 結算退休金（依勞退制度）
- 不需資遣費
...
### 5.3 員工離職Saga流程 [2026-03-17 更新]
```mermaid
flowchart TD
    Start([HR確認離職]) --> SelectType{選擇離職類型}

    SelectType -->|自願離職| Terminate1[標記TERMINATED<br/>terminationType=VOLUNTARY_RESIGNATION]
    SelectType -->|資遣| CalcNotice[計算預告期天數<br/>BR-02-101]
    SelectType -->|懲戒解雇| Terminate3[標記TERMINATED<br/>terminationType=DISMISSAL]
    SelectType -->|退休| Terminate4[標記TERMINATED<br/>terminationType=RETIREMENT]
    SelectType -->|合意終止| Terminate5[標記TERMINATED<br/>terminationType=MUTUAL_AGREEMENT]
    SelectType -->|契約到期| Terminate6[標記TERMINATED<br/>terminationType=CONTRACT_EXPIRY]

    CalcNotice --> CheckReport{資遣日 ≥ 10天後?}
    CheckReport -->|否| AlertHR[提醒HR：需先完成資遣通報<br/>BR-02-105]
    CheckReport -->|是| Terminate2[標記TERMINATED<br/>terminationType=LAYOFF]
    AlertHR --> Terminate2

    Terminate1 & Terminate2 & Terminate3 & Terminate4 & Terminate5 & Terminate6 --> Event[發布EmployeeTerminated事件]

    Event --> Step1[HR01 IAM<br/>停用帳號]
    Event --> Step2[HR05 Insurance<br/>辦理退保]
    Event --> Step3[HR03 Attendance<br/>特休結算 + 補休結算]
    Event --> Step4[HR04 Payroll<br/>最後薪資結算]
    Event --> Step5[HR06 Project<br/>移除/重分配專案成員]
    Event --> Step6[HR07 Timesheet<br/>工時結算]

    Step3 -->|特休折算金額| Step4
    Step4 --> IsLayoff{terminationType == LAYOFF?}
    IsLayoff -->|是| CalcSev[計算資遣費<br/>BR-02-102 / BR-02-103]
    IsLayoff -->|否| FinalPay[產生最終薪資單]
    CalcSev --> GenCert[HR13 Document<br/>產生非自願離職證明<br/>BR-02-104]
    CalcSev --> FinalPay

    Step1 & Step2 & Step5 & Step6 & FinalPay & GenCert --> AllSuccess{全部成功?}
...
### 7.1 EmployeeTerminated 事件（離職）[2026-03-17 更新]
```json
{
  "eventType": "EmployeeTerminated",
  "eventId": "uuid-event",
...
**[2026-03-17 更新] 事件新增欄位說明:**

| 欄位 | 說明 | 用途 |
|:---|:---|:---|
| terminationType | 離職類型枚舉 | 下游服務依此判斷是否需計算資遣費、開立證明 |
| hireDate | 到職日期 | Payroll 計算年資與資遣費使用 |
| averageSalary | 離職前六個月平均工資 | Payroll 計算資遣費基準 |
| noticePeriodDays | 預告期天數 | Payroll 計算未足額預告工資 |
...
**計算規則：**
- 年資起算日 = 到職日（hireDate）
- 年資截止日 = 離職日（terminationDate）
- 未依規定預告者，應給付預告期間工資（由 Payroll 模組計算）
- 預告期工資 = 平均工資 / 30 x 應預告天數
...
**法規依據：** 勞工退休金條例第 12 條（2005/7/1 後適用勞退新制者）

**計算公式：**
```
資遣費 = 平均工資 x 0.5 x 年資（年）
```
...
**適用對象：** 2005/7/1 後到職或已選擇新制的員工

---

### BR-02-103 資遣費計算 — 舊制與混合制

**法規依據：** 勞動基準法第 17 條（舊制）

**舊制計算公式：**
```
資遣費 = 平均工資 x 1.0 x 年資（年）
```
...
**混合制（跨新舊制）：**
```
資遣費 = 舊制年資部分（依舊制）+ 新制年資部分（依新制）
分界日 = 2005/7/1 或員工選擇新制日期
```

> **備註：** 資遣費計算的實際邏輯由 HR04 Payroll 模組的 SeverancePayCalculationDomainService 執行，HR02 僅負責傳遞 terminationType、hireDate、averageSalary 等參數。
...
**規則：**
- 資遣員工時，雇主**應主動**發給非自願離職證明書
- 證明內容須包含：員工姓名、離職日期、離職原因
- 離職原因須載明勞基法條款（如「勞基法第 11 條第 2 款」）
- 整合 HR13 Document 模組產出 PDF

**適用類型：** LAYOFF（資遣）、部分 DISMISSAL（視情況）
...
**規則：**
- 資遣員工前，雇主應向當地主管機關（勞動局/就業服務站）通報
- 通報時間：**離職日 10 天前**
- 系統應於 HR 設定離職日時，自動檢查是否已達通報期限
- 若離職日距今不足 10 天，系統應顯示警告提醒
...
**法規依據：** 勞動基準法第 16 條第 2 項

**規則：**
- 資遣預告期間內，員工得於每週請謀職假 2 日
- 謀職假期間工資照給
- 通知 HR03 Attendance 模組建立謀職假假別

**適用類型：** LAYOFF（資遣）

---

### BR-02-107 離職跨模組連動事件清單

**EmployeeTerminatedEvent 觸發的跨模組動作：**

| 序號 | 目標模組 | 動作 | 實作狀態 | 備註 |
|:---:|:---|:---|:---:|:---|
| 1 | HR01 IAM | 停用使用者帳號 | 待實作 | 離職日當天生效 |
| 2 | HR03 Attendance | 特休結算（折算工資金額）+ 補休結算 | 待實作 | 結算金額傳給 Payroll |
| 3 | HR04 Payroll | 最後薪資結算（當月薪資 + 特休折算 + 資遣費） | 待實作 | 依 terminationType 決定項目 |
| 4 | HR05 Insurance | 自動退保（勞保、健保、勞退） | 已實作 | AutoWithdrawOnTerminationService |
| 5 | HR06 Project | 任務重分配或標記 | 待實作 | 移除專案成員 |
| 6 | HR07 Timesheet | 工時結算 | 待實作 | 結算至離職日 |
| 7 | HR13 Document | 產生非自願離職證明（僅 LAYOFF） | 待實作 | PDF 範本 |
