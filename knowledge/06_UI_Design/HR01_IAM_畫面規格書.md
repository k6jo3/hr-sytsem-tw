...
| **頁面數量** | 4 頁 + 1 NotFoundPage + 2 Modal |
...
| 區域 | 說明 | 元件 |
|:---|:---|:---|
| **左側品牌區（60%）** | 深色漸層背景 `#1a1a2e → #16213e → #0f3460`，品牌 Logo、系統名稱、特色描述，白色文字 | CSS `linear-gradient(135deg, ...)` |
| **右側表單區（40%）** | 淺灰背景 `#f8f9fc`，登入表單居中顯示 | `Form` 容器 |
...
**空狀態（v1.2）：** 當角色列表為空時，顯示 `Empty.PRESENTED_IMAGE_SIMPLE` + 「尚未建立任何角色」提示文字。
...
## 9. HR01-P05 404 Not Found 頁面（v1.2 新增）

### 9.1 頁面概述

| 項目 | 說明 |
|:---|:---|
| **用途** | 當使用者存取不存在的路由時顯示 404 錯誤頁面 |
| **使用角色** | ALL |
| **進入方式** | 存取任何未定義的路由 |
| **頁面類型** | 錯誤頁面（不含 PageLayout） |
...
| 項目 | 說明 |
|:---|:---|
| **元件** | Ant Design `Result` 元件，`status="404"` |
| **標題** | `404` |
| **副標題** | `抱歉，您存取的頁面不存在` |
| **操作按鈕** | `Button type="primary"` → 導向 `/dashboard` |
| **前端檔案** | `frontend/src/pages/NotFoundPage.tsx` |

---

## 10. 系統管理 - 排程管理 Tab（ScheduledJobTab）
