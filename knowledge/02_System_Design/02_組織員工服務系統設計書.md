...
**版本:** 1.1
**日期:** 2025-12-05（[2026-03-17 更新] 新增離職類型區分與法規要件）
...
- **組織選擇器（Grouped Dropdown）**
  - 以 OptGroup 分組顯示：母公司群組與子公司群組
  - 選擇不同組織後自動載入對應的組織樹
  - 格式範例：`[母公司] 母公司名稱 / [子公司] 子公司A、子公司B`
...
sequenceDiagram
    ...
    HR->>Page: 2. 點擊"離職"按鈕
    Page->>HR: 3. 顯示離職確認對話框
    HR->>Page: 4. 選擇離職類型、輸入離職日期、離職原因
    Note right of Page: [2026-03-17 更新] 新增離職類型選擇
    HR->>Page: 5. 確認離職
...
| `E-ORG-04` | 新增公司按鈕 | onClick | 開啟新增公司對話框（當選擇類型為 SUBSIDIARY 時，顯示母公司選擇器 parentOrganizationId） | - |
...
const handleNodeClick = async (node: OrganizationTreeNode) => {
  if (node.type === 'department') {
    try {
      ...
      message.error(error?.response?.data?.message || '載入部門資訊失敗');
    }
  }
};
```
...
const handleSearch = useDebouncedCallback(async (keyword: string) => {
  try {
    ...
    message.error(error?.response?.data?.message || '查詢失敗');
  } finally {
    setLoading(false);
  }
}, 500);
...
const handleQuery = async () => {
  try {
    ...
    message.error(error?.response?.data?.message || '查詢部門列表失敗');
  } finally {
    setLoading(false);
  }
};
...
const handleCreateDepartment = async (values: CreateDepartmentFormData) => {
  try {
    ...
    if (error.code === 'RESOURCE_DEPT_CODE_EXISTS') {
      ...
      message.error(error?.response?.data?.message || '建立部門失敗');
    }
  } finally {
    setSubmitting(false);
  }
...
const handleSubmit = async () => {
  try {
    ...
    if (error.name === 'ValidationError') {
      ...
      message.error(error?.response?.data?.message || '建立失敗: ' + error.message);
    }
  } finally {
    setSubmitting(false);
  }
...
CREATE TABLE employees (
    ...
    termination_type VARCHAR(30),  -- [2026-03-17 更新] 離職類型
    ...
    CONSTRAINT chk_termination_type CHECK (termination_type IS NULL OR termination_type IN ('VOLUNTARY_RESIGNATION', 'LAYOFF', 'DISMISSAL', 'MUTUAL_AGREEMENT', 'CONTRACT_EXPIRY', 'RETIREMENT')),  -- [2026-03-17 更新]
...
public class Employee {
    ...
    public void terminate(LocalDate terminationDate, String reason) {
        terminate(terminationDate, TerminationType.VOLUNTARY_RESIGNATION, reason);
    }

    /**
     * 離職 [2026-03-17 更新] — 含離職類型
     * 法規依據：勞基法第 11、12、14、15、53、54 條
     */
    public void terminate(LocalDate terminationDate, TerminationType terminationType, String reason) {
        ...
        if (terminationType == TerminationType.LAYOFF) {
            noticePeriodDays = calculateNoticePeriod();
        }
    ...
    /**
     * [2026-03-17 更新] 計算資遣預告期天數
     * 法規依據：勞動基準法第 16 條
     */
    public int calculateNoticePeriod() {
        long serviceMonths = ChronoUnit.MONTHS.between(this.hireDate, this.terminationDate);
        if (serviceMonths < 3) {
            return 0; // 未滿3個月免預告
    ...
    /**
     * [2026-03-17 更新] 計算年資（含年與月）
     */
    public ServiceYears getServiceYears() {
        if (this.terminationDate == null) {
            return ServiceYears.from(this.hireDate, LocalDate.now());
        }
        return ServiceYears.from(this.hireDate, this.terminationDate);
    }
...
| `EmployeeTerminated` | 員工離職 [2026-03-17 更新: 含 terminationType] | Organization | IAM, Attendance, Insurance, Payroll, Project, Timesheet |
...
#### 8.2.2 EmployeeTerminatedEvent (關鍵事件) [2026-03-17 更新]
...
public class EmployeeTerminatedEvent implements DomainEvent {
    ...
    private String employeeNumber;
    private LocalDate terminationDate;
    private TerminationType terminationType;  // [2026-03-17 更新]
    private String reason;
    private LocalDate hireDate;               // [2026-03-17 更新] 年資計算用
    private int noticePeriodDays;             // [2026-03-17 更新] 預告期天數
}
```
...
### A.7.1 離職類型 (TerminationType) [2026-03-17 更新]
...
### A.7.2 ServiceYears 值對象 [2026-03-17 更新]
...
public class ServiceYears {
    ...
    public static ServiceYears from(LocalDate startDate, LocalDate endDate) {
        Period period = Period.between(startDate, endDate);
        long totalDays = ChronoUnit.DAYS.between(startDate, endDate);
        return new ServiceYears(period.getYears(), period.getMonths(), period.getDays(), totalDays);
    }

    /** 轉為年（含小數），用於資遣費計算 */
    public BigDecimal toDecimalYears() {
