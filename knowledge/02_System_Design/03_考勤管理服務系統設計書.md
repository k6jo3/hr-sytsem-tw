...
erDiagram
    ...
    shifts ||--o{ shift_schedules : "排班"
    employees ||--o{ shift_schedules : "被排班"
    rotation_patterns ||--o{ shift_schedules : "輪班模式"
    rotation_patterns ||--o{ rotation_days : "天序"
...
CREATE INDEX idx_schedule_employee ON shift_schedules(employee_id);
CREATE INDEX idx_schedule_date ON shift_schedules(schedule_date);

COMMENT ON TABLE shift_schedules IS '排班表';
COMMENT ON COLUMN shift_schedules.status IS '排班狀態: DRAFT草稿, PUBLISHED已發佈, LOCKED已鎖定';
...
CREATE INDEX idx_rotation_org ON rotation_patterns(organization_id);

COMMENT ON TABLE rotation_patterns IS '輪班模式表';
COMMENT ON COLUMN rotation_patterns.cycle_days IS '一個完整循環的天數';

-- 輪班天序表
CREATE TABLE rotation_days (
    day_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    pattern_id UUID NOT NULL REFERENCES rotation_patterns(pattern_id),
...
COMMENT ON TABLE rotation_days IS '輪班天序表（循環中每天的班別）';
...
CREATE TABLE shift_swap_requests (
    ...
    requester_shift_id UUID REFERENCES shifts(shift_id),
    counterpart_shift_id UUID REFERENCES shifts(shift_id),
    status VARCHAR(30) NOT NULL DEFAULT 'PENDING_COUNTERPART'
        CHECK (status IN ('PENDING_COUNTERPART', 'PENDING_APPROVAL', 'APPROVED', 'REJECTED', 'CANCELLED')),
    reason TEXT,
    rejection_reason TEXT,
    approver_id UUID,
    is_deleted BOOLEAN DEFAULT FALSE,
...
CREATE INDEX idx_swap_counterpart ON shift_swap_requests(counterpart_id);
CREATE INDEX idx_swap_status ON shift_swap_requests(status);

COMMENT ON TABLE shift_swap_requests IS '換班申請表';
COMMENT ON COLUMN shift_swap_requests.status IS '換班狀態: PENDING_COUNTERPART等待對方同意, PENDING_APPROVAL等待審核, APPROVED核准, REJECTED駁回, CANCELLED取消';
```
...
#### 7.1.7 ShiftSchedule聚合根 (排班表)

**職責:** 記錄員工在某日被指派的班別，支援手動排班與輪班模式自動排班。

**欄位:**
- `id: ScheduleId` — 排班表 ID
- `employeeId: String` — 員工 ID
- `shiftId: ShiftId` — 班別 ID
- `scheduleDate: LocalDate` — 排班日期
- `status: ScheduleStatus` — 排班狀態 (DRAFT/PUBLISHED/LOCKED)
- `rotationPatternId: String` — 輪班模式 ID（手動排班為 null）
- `note: String` — 備註

**Domain 行為:**
- `changeShift(ShiftId)` — 變更班別（LOCKED 狀態不可變更）
- `publish()` — 發佈排班（DRAFT → PUBLISHED）
- `lock()` — 鎖定排班（PUBLISHED → LOCKED）
- `revertToDraft()` — 退回草稿

#### 7.1.8 RotationPattern聚合根 (輪班模式)

**職責:** 定義循環的班別序列（如：早班→中班→晚班→休息→早班...）

**欄位:**
- `id: RotationPatternId` — 輪班模式 ID
- `organizationId: String` — 組織 ID
- `name/code: String` — 名稱/代碼
- `cycleDays: int` — 循環天數
- `rotationDays: List<RotationDay>` — 循環天序（每天的班別或休息）

**Domain 行為:**
- `setRotationDays(List<RotationDay>)` — 整批設定天序
- `getDayForIndex(int dayIndex)` — 取得某日應排的班別
...
#### 7.1.9 ShiftSwapRequest聚合根 (換班申請)

**職責:** 管理員工間的換班流程（申請 → 對方同意 → 主管審核）

**狀態流轉:**
```
PENDING_COUNTERPART → acceptByCounterpart() → PENDING_APPROVAL
PENDING_COUNTERPART → rejectByCounterpart() → REJECTED
PENDING_APPROVAL → approve() → APPROVED
PENDING_APPROVAL → reject() → REJECTED
(任何未結案狀態) → cancel() → CANCELLED
```

#### 7.1.10 ShiftSchedulingDomainService (排班 Domain Service)

**職責:** 根據輪班模式自動產生排班表、執行換班交換

**方法:**
- `generateSchedules(employeeId, pattern, startDate, endDate, rotationStart)` — 自動排班產生
- `executeSwap(scheduleA, scheduleB)` — 執行兩筆排班的班別交換
...
| `CompTimeGrantedEvent` | 加班核准選擇補休 [2026-03-17 新增] | Attendance | Attendance (self) |
| `CompTimeExpiredEvent` | 補休到期自動折算 [2026-03-17 新增] | Attendance | Payroll |
| `CompTimeExpiringEvent` | 補休即將到期 [2026-03-17 新增] | Attendance | Notification |
| `CompTimeSettlementEvent` | 離職補休結算 [2026-03-17 新增] | Attendance | Payroll |
| `AnnualLeaveSettlementEvent` | 離職特休結算 [2026-03-17 新增] | Attendance | Payroll |
...
## 11. 特休/補休法規合規性設計 [2026-03-17 更新]

### 11.1 AnnualLeaveRule 非整數年資支援

**問題：** 現有 `AnnualLeaveRule` 的 `minServiceYears` / `maxServiceYears` 雖已宣告為 `DECIMAL(4,2)`，但 Domain 計算邏輯中以 `int` 處理年資，無法正確處理 6 個月（0.5 年）的情境。

**設計變更：**

#### 11.1.1 AnnualLeaveRule 欄位調整

保留現有 `min_service_years DECIMAL(4,2)` / `max_service_years DECIMAL(4,2)` 資料庫欄位不變，但 Domain 層計算需改用 `BigDecimal` 精確比對：

```java
/**
 * 特休規則 — 支援非整數年資（如 0.5 年 = 6 個月）
 * [2026-03-17 更新] 修正年資比對邏輯以符合勞基法第 38 條
 */
@Entity
@Table(name = "annual_leave_rules")
public class AnnualLeaveRule {
    ...
    // 已為 BigDecimal，無需修改資料庫
    @Column(name = "min_service_years", nullable = false, precision = 4, scale = 2)
    private BigDecimal minServiceYears;

    @Column(name = "max_service_years", precision = 4, scale = 2)
    private BigDecimal maxServiceYears; // null 表示無上限（10 年以上）
    ...
    /**
     * 檢查年資是否落在此規則區間
     * 區間為 [minServiceYears, maxServiceYears)，左閉右開
     */
    public boolean matches(BigDecimal serviceYears) {
        boolean aboveMin = serviceYears.compareTo(minServiceYears) >= 0;
        boolean belowMax = maxServiceYears == null
            || serviceYears.compareTo(maxServiceYears) < 0;
        return aboveMin && belowMax;
    }
}
```

#### 11.1.2 AnnualLeavePolicy 計算邏輯修正

```java
/**
 * 計算特休天數 (依勞基法第 38 條)
 * [2026-03-17 更新] 修正為精確年資比對，支援非整數年資
 */
public int calculateAnnualLeaveDays(LocalDate hireDate, LocalDate currentDate) {
    BigDecimal serviceYears = calculateServiceYears(hireDate, currentDate);
    ...
    // 依規則表逐一比對
    for (AnnualLeaveRule rule : rules) {
        if (rule.matches(serviceYears)) {
            // 10 年以上特殊邏輯：base 15 天 + 每年加 1 天，上限 30 天
            if (serviceYears.compareTo(BigDecimal.TEN) >= 0) {
                int extraYears = serviceYears.intValue() - 10;
                return Math.min(rule.getLeaveDays() + extraYears, 30);
            }
            return rule.getLeaveDays();
        }
    }
 ...
 * [2026-03-17 更新]
 */
private BigDecimal calculateServiceYears(LocalDate hireDate, LocalDate currentDate) {
    long totalMonths = ChronoUnit.MONTHS.between(hireDate, currentDate);
    return new BigDecimal(totalMonths)
...
```sql
-- [2026-03-17 更新] 確認 min_service_years 使用精確小數
-- 現有 SQL 已使用 0.5，無需變更，但需確認 10 年以上的 leave_days = 15（base）
-- 10 年以上每年 +1 天由程式邏輯處理，資料庫只記錄 base 值
UPDATE annual_leave_rules
SET leave_days = 15
WHERE min_service_years = 10.00
  AND policy_id = (SELECT policy_id FROM annual_leave_policies LIMIT 1);
```

### 11.2 特休使用提醒機制設計

**法規依據：** 勞基法第 38 條第 3 項 — 雇主應主動提醒勞工排定特休。

#### 11.2.1 排程設計 — AnnualLeaveReminderJob

```mermaid
flowchart TD
    Start([每日排程 09:00]) --> Query[查詢所有特休餘額]
    Query --> Filter{篩選即將到期}
    Filter -->|到期前 30 天| Remind30[發布提醒事件 LEVEL_1]
    Filter -->|到期前 14 天| Remind14[發布提醒事件 LEVEL_2]
    Filter -->|到期前 7 天| Remind7[發布提醒事件 LEVEL_3]
    Remind30 --> Notify[Notification Service]
    Remind14 --> Notify
    Remind7 --> Notify
    Notify --> Done([完成])

    Remind30 -.- Note30[通知對象：員工]
    Remind14 -.- Note14[通知對象：員工 + 主管]
    Remind7 -.- Note7[通知對象：員工 + 主管 + HR]
```
...
```json
{
  ...
  "payload": {
    ...
    "notifyTargets": ["EMPLOYEE", "MANAGER"]
  }
}
```
...
sequenceDiagram
    ...
    Note over Org: 員工離職作業
    Org->>Kafka: Publish EmployeeTerminatedEvent

    Kafka->>Att: Subscribe
    Note over Att: 特休/補休結算
    ...
    Att->>Kafka: Publish AnnualLeaveSettlementEvent
    Att->>Kafka: Publish CompTimeSettlementEvent

    Kafka->>Pay: Subscribe
    Pay->>Pay: 納入最後薪資結算
```

#### 11.3.2 結算 Domain Service

```java
/**
 * 離職特休/補休結算 Domain Service
 * [2026-03-17 新增]
 */
public class TerminationLeaveSettlementDomainService {

    /**
     * 計算特休折算金額
     * @param remainingDays 剩餘特休天數
     * @param dailyWage 日薪（月薪 / 30）
     * @return 折算金額
     */
    public BigDecimal calculateAnnualLeaveSettlement(
            BigDecimal remainingDays, BigDecimal dailyWage) {
        if (remainingDays.compareTo(BigDecimal.ZERO) <= 0) {
            return BigDecimal.ZERO;
        }
        return remainingDays.multiply(dailyWage)
            .setScale(0, RoundingMode.CEILING); // 無條件進位至整數
    }

    /**
     * 計算補休折算金額
     * @param compTimeBalances 所有未結算的補休餘額
     * @return 折算總金額
     */
    public BigDecimal calculateCompTimeSettlement(
            List<CompTimeBalance> compTimeBalances) {
        return compTimeBalances.stream()
            .filter(b -> b.getRemainingHours().compareTo(BigDecimal.ZERO) > 0)
            .map(CompTimeBalance::calculateSettlementAmount)
...
#### 11.3.3 Application Service Pipeline

```java
/**
 * 離職結算 Service — 監聽 EmployeeTerminatedEvent
 * [2026-03-17 新增]
 *
 * Pipeline Tasks:
 * 1. LoadTerminationDataTask  — 載入員工特休/補休餘額
 * 2. CalculateSettlementTask  — 計算折算金額
 * 3. MarkBalancesSettledTask  — 標記餘額為已結算
 * 4. PublishSettlementEventsTask — 發布結算事件給 Payroll
 */
@Service("terminationLeaveSettlementServiceImpl")
public class TerminationLeaveSettlementServiceImpl {
    // 由 EmployeeTerminatedEvent 的 Kafka Consumer 觸發
}
```

### 11.4 補休餘額（CompTimeBalance）設計
...
CREATE TABLE comp_time_balances (
    balance_id UUID PRIMARY KEY DEFAULT gen_random_uuid(),
    employee_id UUID NOT NULL,
    overtime_application_id UUID NOT NULL REFERENCES overtime_applications(overtime_id),

    -- 來源加班資訊
    overtime_date DATE NOT NULL,
    overtime_type VARCHAR(20) NOT NULL
        CHECK (overtime_type IN ('WEEKDAY', 'REST_DAY', 'HOLIDAY')),
    overtime_hours DECIMAL(4,1) NOT NULL,
    hourly_wage_rate DECIMAL(10,2) NOT NULL,

    -- 補休額度
    granted_hours DECIMAL(4,1) NOT NULL,
    ...
    settlement_amount DECIMAL(10,0),
    settled_at TIMESTAMP,

    created_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    updated_at TIMESTAMP DEFAULT CURRENT_TIMESTAMP,

    CONSTRAINT chk_comp_hours CHECK (used_hours <= granted_hours),
    CONSTRAINT chk_comp_ot_type CHECK (overtime_type IN ('WEEKDAY', 'REST_DAY', 'HOLIDAY'))
);

CREATE INDEX idx_comp_time_employee ON comp_time_balances(employee_id);
CREATE INDEX idx_comp_time_expiry ON comp_time_balances(expiry_date)
    WHERE is_expired = FALSE AND is_settled = FALSE;
CREATE INDEX idx_comp_time_overtime ON comp_time_balances(overtime_application_id);

COMMENT ON TABLE comp_time_balances IS '補休餘額表 [2026-03-17 新增]';
COMMENT ON COLUMN comp_time_balances.hourly_wage_rate IS '加班當日的時薪費率，供到期折算使用';
COMMENT ON COLUMN comp_time_balances.expiry_date IS '補休到期日，依勞基法第32-1條不得超過年度終結';
COMMENT ON COLUMN comp_time_balances.settlement_amount IS '到期/離職折算金額';
```

#### 11.4.2 ER Diagram 補充

```mermaid
erDiagram
    employees ||--o{ comp_time_balances : "補休餘額"
    overtime_applications ||--o| comp_time_balances : "來源加班"

    comp_time_balances {
        uuid balance_id PK
        uuid employee_id FK
        uuid overtime_application_id FK
        date overtime_date
        varchar overtime_type
        decimal overtime_hours
        decimal hourly_wage_rate
        decimal granted_hours
        decimal used_hours
        date expiry_date
        boolean is_expired
        boolean is_settled
...
#### 11.4.3 CompTimeBalance Domain 設計
...
public class CompTimeBalance {
    @EmbeddedId
    private CompTimeBalanceId id;

    @Column(name = "employee_id", nullable = false)
    private UUID employeeId;

    @Column(name = "overtime_application_id", nullable = false)
    private UUID overtimeApplicationId;
    ...
    @Column(name = "hourly_wage_rate", nullable = false)
    private BigDecimal hourlyWageRate;

    @Column(name = "granted_hours", nullable = false)
    ...
    public BigDecimal getRemainingHours() {
        return grantedHours.subtract(usedHours);
    }
     ...
     * 建立補休餘額（加班核准且選擇補休時呼叫）
     */
    public static CompTimeBalance create(
            UUID employeeId,
            OvertimeApplication overtime,
            BigDecimal hourlyWageRate,
            LocalDate expiryDate) {

        CompTimeBalance balance = new CompTimeBalance();
        balance.id = CompTimeBalanceId.generate();
        balance.employeeId = employeeId;
        balance.overtimeApplicationId = overtime.getId().getValue();
        balance.overtimeDate = overtime.getOvertimeDate();
        balance.overtimeType = overtime.getOvertimeType();
        balance.overtimeHours = overtime.getOvertimeHours();
        balance.hourlyWageRate = hourlyWageRate;
        balance.grantedHours = overtime.getOvertimeHours(); // 1:1 換算
        balance.usedHours = BigDecimal.ZERO;
        balance.expiryDate = expiryDate;
        balance.isExpired = false;
        balance.isSettled = false;
    ...
    public void use(BigDecimal hours) {
        if (this.isExpired || this.isSettled) {
            throw new DomainException("此補休已到期或已結算，無法使用");
        }
        BigDecimal remaining = getRemainingHours();
        if (remaining.compareTo(hours) < 0) {
            throw new DomainException(
                String.format("補休時數不足，剩餘 %s 小時，申請 %s 小時",
                    remaining, hours));
        }
    ...
    /**
     * 到期處理 — 未休時數折算為加班費
     * 折算金額 = 剩餘時數 x 時薪費率 x 加班費率倍數
     */
    public BigDecimal expire() {
        if (this.isSettled) {
            throw new DomainException("此補休已結算");
        }
     ...
     * 離職結算 — 邏輯同到期處理
     */
    public BigDecimal settleOnTermination() {
        return expire();
    }

    /**
     * 計算折算金額
     * 依加班當日工資費率折算（勞基法第 32-1 條）
     */
    public BigDecimal calculateSettlementAmount() {
        BigDecimal remaining = getRemainingHours();
        BigDecimal overtimeMultiplier = getOvertimeMultiplier(this.overtimeType);
        return remaining.multiply(hourlyWageRate)
    ...
    /**
     * 取得加班費率倍數
     * 平日延長工時前 2 小時: 1.34, 後 2 小時: 1.67
     * 休息日: 前 2 小時 1.34, 2~8 小時 1.67, 8 小時以上 2.67
     * 國定假日: 2.0
     * 簡化處理：使用加權平均倍數
     */
    private BigDecimal getOvertimeMultiplier(OvertimeType type) {
        switch (type) {
            case WEEKDAY:  return new BigDecimal("1.34");
            case REST_DAY: return new BigDecimal("1.34");
            case HOLIDAY:  return new BigDecimal("2.00");
            default:       return new BigDecimal("1.34");
        }
    }
     ...
     */
    public boolean isExpiringSoon(int daysThreshold) {
        if (this.isExpired || this.isSettled) return false;
        if (getRemainingHours().compareTo(BigDecimal.ZERO) <= 0) return false;
...
#### 11.4.4 CompTimeBalance Repository
 ...
 * [2026-03-17 新增]
 */
public interface ICompTimeBalanceRepository {
    CompTimeBalance findById(CompTimeBalanceId id);
    List<CompTimeBalance> findByEmployeeId(UUID employeeId);
    List<CompTimeBalance> findActiveByEmployeeId(UUID employeeId);
    List<CompTimeBalance> findExpiredUnsettled(LocalDate asOfDate);
    List<CompTimeBalance> findExpiringSoon(LocalDate beforeDate);
    void save(CompTimeBalance balance);
}
```

### 11.5 排程任務設計

#### 11.5.1 CompTimeExpiryJob（補休到期自動折算）

```mermaid
flowchart TD
    Start([每日 00:30 排程]) --> Query[查詢到期且未結算的補休]
    Query --> HasData{有到期資料?}
    HasData -->|否| Done([結束])
    HasData -->|是| Loop[逐筆處理]
    Loop --> Calc[計算折算金額]
    Calc --> Settle[標記已結算]
    Settle --> Publish[發布 CompTimeExpiredEvent]
    Publish --> Next{還有下一筆?}
    Next -->|是| Loop
    Next -->|否| Done
```
...
#### 11.5.2 CompTimeReminderJob（補休到期提醒）

**排程設定:** `cron: 0 0 9 * * ?`

**流程：**
1. 查詢 7 天內即將到期、剩餘時數 > 0 的補休
2. 發布 `CompTimeExpiringEvent`
3. Notification Service 發送提醒

### 11.6 OvertimeApplication 聯動修改

加班申請核准且選擇補休時，需自動建立 CompTimeBalance：

```java
/**
 * 加班核准後處理
 * [2026-03-17 更新] 補充補休額度建立邏輯
 */
public void approve(UUID approverId) {
    if (this.status != ApplicationStatus.PENDING) {
        throw new DomainException("只能核准待審核的申請");
    ...
    // 若選擇補休，發布事件通知建立 CompTimeBalance
    if (this.compensationType == CompensationType.COMP_TIME) {
        DomainEventPublisher.publish(new CompTimeGrantedEvent(
            this.id.getValue(),
            this.employeeId,
            this.overtimeDate,
            this.overtimeHours,
...
| 事件名稱 | 觸發時機 | 發布服務 | 訂閱服務 |
|:---|:---|:---|:---|
| `CompTimeGrantedEvent` | 加班核准選擇補休 | Attendance | Attendance (self) |
| `CompTimeExpiredEvent` | 補休到期自動折算 | Attendance | Payroll |
| `CompTimeExpiringEvent` | 補休即將到期 | Attendance | Notification |
| `CompTimeSettlementEvent` | 離職補休結算 | Attendance | Payroll |
| `AnnualLeaveSettlementEvent` | 離職特休結算 | Attendance | Payroll |

### 11.8 新增事件 Schema

#### 11.8.1 AnnualLeaveSettlementEvent

```json
{
  "eventType": "AnnualLeaveSettlement",
  ...
  "payload": {
    ...
    "remainingDays": 5.0,
    "dailyWage": 1667,
    "settlementAmount": 8335,
    "leaveYear": 2026
  }
}
```

#### 11.8.2 CompTimeExpiredEvent

```json
{
  "eventType": "CompTimeExpired",
  "timestamp": "2026-03-17T00:30:00Z",
  "payload": {
    "balanceId": "uuid-balance",
    "employeeId": "uuid-emp",
    ...
    "settlementAmount": 837
  }
}
```

#### 11.8.3 CompTimeSettlementEvent

```json
{
  "eventType": "CompTimeSettlement",
  "timestamp": "2026-03-15T10:00:00Z",
  "payload": {
    "employeeId": "uuid-emp",
    "terminationDate": "2026-03-31",
...
15. CompTimeBalance 聚合根與 Repository
16. TerminationLeaveSettlementDomainService（離職結算 Domain Service）
17. TerminationLeaveSettlementServiceImpl（監聽 EmployeeTerminatedEvent）
18. CompTimeExpiryJob 排程（每日檢查到期補休）
19. CompTimeReminderJob 排程（補休到期提醒）
20. AnnualLeaveReminderJob 排程修正（多級提醒 30/14/7 天）
21. OvertimeApplication 核准後補休建立邏輯
22. AnnualLeavePolicy.calculateAnnualLeaveDays() 修正非整數年資

#### 資料庫開發工項（新增）

5. 建立 `comp_time_balances` 資料表 DDL
6. 建立相關索引
7. 確認 `annual_leave_rules` 的 10 年以上規則 base 值為 15

---

**文件完成日期:** 2025-12-06
**版本:** 1.1（2026-03-17 補充特休/補休法規合規性設計）
