...
## Coding注意事項
1. 採用TDD的開發流程
2. 全部的程式含架構設計，需符合SOLID與clean code的原則
...
15. 整合測試只在api的端點測試執行
16. 測試重點在於合約測試、domain以及business pipeline的單元測試、api的整合測試
17. 有測試失敗的部份，需先確認是測試本身寫錯還是程式邏輯錯誤或是合約測試文件內容有錯誤，看是那邊有錯就修那邊，先在需修正的部份留下todo說明要修正什麼即可，不用立即修正
18. 合約測試的重點在於案例的正確，而不是sql的生成是否符合
---
### 總結架構師的實踐清單
