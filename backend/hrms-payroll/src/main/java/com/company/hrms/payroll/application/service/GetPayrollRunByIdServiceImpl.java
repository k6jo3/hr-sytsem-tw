 ...
 * </p>
 */
@Service("getPayrollRunByIdServiceImpl")
@Transactional(readOnly = true)
@RequiredArgsConstructor
public class GetPayrollRunByIdServiceImpl extends AbstractQueryService<String, PayrollRunResponse> {
    ...
    @Override
    protected PayrollRunResponse executeQuery(QueryGroup query, String id, JWTModel currentUser, String... args)
            throws Exception {
        ...
        PayrollRun run = repository.findById(new RunId(targetId))
                .orElseThrow(() -> new DomainException("PAYROLL_RUN_NOT_FOUND", "Payroll run not found: " + targetId));

        return PayrollRunDtoFactory.toResponse(run);
    }
}
