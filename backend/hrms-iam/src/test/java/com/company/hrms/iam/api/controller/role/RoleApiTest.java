...
class RoleApiTest extends BaseApiContractTest {
    ...
    class RoleCommandApiTests {
        ...
        void updateRole_ShouldReturnUpdatedRole() throws Exception {
            ...
            when(updateRoleService.execCommand(any(UpdateRoleRequest.class), any(JWTModel.class), eq(roleId)))
                    .thenReturn(response);
        ...
        void assignPermissions_ShouldReturn204() throws Exception {
            // Arrange
            String roleId = "role-001";
            HR01RoleCmdController.AssignPermissionsRequest request =
                    new HR01RoleCmdController.AssignPermissionsRequest(
                            Arrays.asList("perm-001", "perm-002", "perm-003"));

            doNothing().when(assignPermissionsService).execCommand(
                    any(HR01RoleCmdController.AssignPermissionsRequest.class),
                    any(JWTModel.class),
                    eq(roleId));
    ...
    class RoleQueryApiTests {
        ...
        void getRoleList_ShouldReturnList() throws Exception {
            ...
            when(getRoleListService.getResponse(any(HR01RoleQryController.RoleQueryRequest.class), any(JWTModel.class)))
        ...
        void getRole_ShouldReturnRoleDetail() throws Exception {
            ...
            when(getRoleService.getResponse(any(HR01RoleQryController.GetRoleRequest.class), any(JWTModel.class),
                    eq(roleId)))
                    .thenReturn(response);
            ...
            performGet("/api/v1/roles/" + roleId)
                    .andExpect(status().isOk())
                    .andExpect(jsonPath("$.roleId").value(roleId))
                    .andExpect(jsonPath("$.roleName").value("管理員"))
