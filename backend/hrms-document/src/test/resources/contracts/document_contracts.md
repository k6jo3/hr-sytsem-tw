...
| DOC_D007 | 依標籤查詢 | EMPLOYEE | `{"tag":"合約"}` | `tags LIKE '合約'`, `is_deleted = 0`, `visibility IN [PUBLIC, SHARED, DEPARTMENT]` |
| DOC_D008 | 查詢最近文件 | EMPLOYEE | `{"days":7}` | `is_deleted = 0`, `owner_id = 'currentUserId'`, `visibility IN [PUBLIC, SHARED, DEPARTMENT]`, `updated_at >= 2025-01-01` |
...
| 場景 ID | 測試描述 | 模擬角色 | 輸入 (Request) | 必須包含的過濾條件 (Required Filters) |
| :--- | :--- | :--- | :--- | :--- |
| DOC_F001 | 查詢根資料夾 | EMPLOYEE | `{"parentId":null}` | `parent_id IS NULL`, `is_deleted = 0`, `visibility IN [PUBLIC, SHARED, DEPARTMENT]` |
| DOC_F002 | 查詢子資料夾 | EMPLOYEE | `{"parentId":"F001"}` | `parent_id = 'F001'`, `is_deleted = 0` |
| DOC_F003 | 查詢個人資料夾 | EMPLOYEE | `{}` | `is_deleted = 0`, `owner_id = 'currentUserId'` |
| DOC_F004 | 依名稱查詢 | EMPLOYEE | `{"name":"專案"}` | `name LIKE '專案'`, `is_deleted = 0`, `visibility IN [PUBLIC, SHARED, DEPARTMENT]` |
