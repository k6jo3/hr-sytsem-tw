...
public class PerformanceApiContractTest extends BaseApiContractTest {
    ...
    class CycleApiContractTests {
        ...
        void getCycles_InProgress_ShouldIncludeFilters() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/performance/cycles?status=IN_PROGRESS")
                    .requestAttr("currentUser", mockUser)
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk());

            QueryGroup query = queryCaptor.getValue();
            assertContract(query, contractSpec, "PFM_C001");
        }
        ...
        void getCycles_Annual_ShouldIncludeFilters() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/performance/cycles?cycleType=ANNUAL")
                    .requestAttr("currentUser", mockUser)
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk());

            QueryGroup query = queryCaptor.getValue();
            assertContract(query, contractSpec, "PFM_C002");
        }
        ...
        void getCycles_ByYear_ShouldIncludeFilters() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/performance/cycles?year=2025")
                    .requestAttr("currentUser", mockUser)
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk());

            QueryGroup query = queryCaptor.getValue();
            assertContract(query, contractSpec, "PFM_C003");
        }
    }
    ...
    class ReviewApiContractTests {
        ...
        void getMyReviews_ShouldIncludeFilters() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/performance/reviews/my")
                    .requestAttr("currentUser", mockUser)
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk());

            QueryGroup query = queryCaptor.getValue();
            String processedSpec = contractSpec.replace("{value}", mockUser.getUserId());
            assertContract(query, processedSpec, "PFM_R001");
        }
        ...
        void getTeamReviews_ShouldIncludeFilters() throws Exception {
            ...
            mockMvc.perform(get("/api/v1/performance/reviews/team")
                    .requestAttr("currentUser", mockUser)
                    .contentType(MediaType.APPLICATION_JSON))
                    .andExpect(status().isOk());
            ...
            QueryGroup query = queryCaptor.getValue();
            String processedSpec = contractSpec.replace("{value}", mockUser.getUserId());
            assertContract(query, processedSpec, "PFM_T001");
        }
    }
}
