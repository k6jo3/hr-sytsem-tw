...
class PerformanceReviewRepositoryIntegrationTest {
    ...
    class StatusQueryTests {
        ...
        void findAll_PendingSelf_ShouldReturnPendingSelfReviews() {
            ...
            // When
            Page<PerformanceReview> result = performanceReviewRepository.findAll(query, PageRequest.of(0, 100));
    ...
    class CycleQueryTests {
        ...
        void findAll_Cycle2025_ShouldReturnCycle2025Reviews() {
            // Given
            QueryGroup query = QueryBuilder.where()
                    .eq("cycle_id", CYCLE_2025.toString())
                    .build();
        ...
        void findAll_Cycle2024_ShouldReturnCycle2024Reviews() {
            // Given
            QueryGroup query = QueryBuilder.where()
                    .eq("cycle_id", CYCLE_2024.toString())
                    .build();
    ...
    class EmployeeQueryTests {
        ...
        void findAll_ByEmployee_ShouldReturnEmployeeReviews() {
            // Given
            QueryGroup query = QueryBuilder.where()
                    .eq("employee_id", EMPLOYEE_01.toString())
                    .build();
    ...
    class PaginationTests {
        ...
        void findAll_Page0_ShouldReturnFirstPage() {
            ...
            // When
            Page<PerformanceReview> result = performanceReviewRepository.findAll(query, PageRequest.of(0, 5));
        ...
        void findAll_Page1_ShouldReturnSecondPage() {
            ...
            // When
            Page<PerformanceReview> result = performanceReviewRepository.findAll(query, PageRequest.of(1, 5));
    ...
    class StatusStatisticsTests {
        ...
        void statusStatistics_ShouldBeCorrect() {
            // When
            QueryGroup pendingSelf = QueryBuilder.where().eq("status", ReviewStatus.PENDING_SELF.name()).build();
            QueryGroup pendingManager = QueryBuilder.where().eq("status", ReviewStatus.PENDING_MANAGER.name()).build();
            QueryGroup pendingFinalize = QueryBuilder.where().eq("status", ReviewStatus.PENDING_FINALIZE.name()).build();
            QueryGroup finalized = QueryBuilder.where().eq("status", ReviewStatus.FINALIZED.name()).build();

            Page<PerformanceReview> pendingSelfReviews = performanceReviewRepository.findAll(pendingSelf, PageRequest.of(0, 100));
            Page<PerformanceReview> pendingManagerReviews = performanceReviewRepository.findAll(pendingManager, PageRequest.of(0, 100));
            Page<PerformanceReview> pendingFinalizeReviews = performanceReviewRepository.findAll(pendingFinalize, PageRequest.of(0, 100));
            Page<PerformanceReview> finalizedReviews = performanceReviewRepository.findAll(finalized, PageRequest.of(0, 100));
